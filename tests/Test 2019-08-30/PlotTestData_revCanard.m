function PlotTestData_revCanard
...
% % test times in seconds wrt 0000 hrs
% NOTE: These are pulled by hand by plotting all GPS data and 
% looking for up/down from altitude data
% [1, 2, 3, 4]
% 1 = the start of the deployment to float
% 2 = the start of the float deployment
% 3 = the end of float (start reeling down)
% 4 = the end of retrieval
ascent_float_descent_1 = [5.649, 5.690, 5.720, 5.741]*(10^4);   % pos
ascent_float_descent_2 = [5.754, 5.767, 5.800, 5.832]*(10^4);   % zero
GPSfix = [5.250, 6.000]*(10^4);     % used for GND station fix estimation

% directories - classic sail only
% 1 = canard, 2 = upsidedown, 3 = classic
% no OG ground station data for this day
fldr = 'plots-canard';
dirsGPS = {[];
           'gondoGPS_1.mat';
           'sailGPS_1.mat';
...
% magnetic declination for RDU for test day
...
for i1 = 1:length(dirsAO)     % skip ground station since not sure parse
    if isempty(dirsAO{i1}) ~= 1
        ...
        switch i1
            ...
            case 3
                AOstruct.Sail = s1.AOdata;
            case 4
                AOstruct.XGND = s1.AOdata;
        end
    end
end


% for compatability since no ground station 
AOstruct.OGGS = AOstruct.XGND;
GPSstruct.OGGS = GPSstruct.XGND;
...
[~, in1, in2] = Subset(GPSstruct.XGND.GPGGA(:, 11), GPSstruct.XGND.GPGGA(:, 4), GPSfix(1), GPSfix(2));
...
% make the plot with all three
neg_xyz = NaN;
MakeXYZPlot(pos_xyz, zro_xyz, neg_xyz, 'X [m]', 'Y [m]', {'Baln +VE', 'Baln 0', 'Baln -VE', ...
    'Sail +VE', 'Sail 0', 'Sail -VE'});
SavePlot(fldr, 'XYZ-all');
...
function SavePlot(fldr, filename)
...
function MakeXYZPlot(pos, zro, neg, xlabs, ylabs, lgnd)
% function to make the XYZ overlap plot
...
figure('Color', 'w');
hold on
plot(pos.Gondo(:, 2), pos.Gondo(:, 3), 'o', 'DisplayName', 'Baln +VE');
plot(zro.Gondo(:, 2), zro.Gondo(:, 3), 'o', 'DisplayName', 'Baln  0 ');
plot(neg.Gondo(:, 2), neg.Gondo(:, 3), 'o', 'DisplayName', 'Baln -VE');
set(gca, 'ColorOrderIndex', 1);
plot(pos.Sail(:, 2), pos.Sail(:, 3), '.', 'DisplayName', 'Sail +VE');
plot(zro.Sail(:, 2), zro.Sail(:, 3), '.', 'DisplayName', 'Sail  0 ');
plot(neg.Sail(:, 2), neg.Sail(:, 3), '.', 'DisplayName', 'Sail -VE');
...
function [outstr] = NanCheck(instr)
% function to take a structure input argument and check for nan

% make nan structure
nanstr.TsGondo = 1; 
nanstr.TeGondo = 3;
nanstr.Gondo_tme = NaN(10, 1);
nanstr.Gondo = NaN(10, 3);
nanstr.TsSail = nanstr.TsGondo; 
nanstr.TeSail = nanstr.TeGondo;
nanstr.Sail = nanstr.Gondo;
nanstr.Sail_tme = nanstr.Gondo_tme;
nanstr.TsGround = nanstr.TsGondo; 
nanstr.TeGround = nanstr.TeGondo;
nanstr.Ground_tme = nanstr.Gondo_tme;
nanstr.Ground = nanstr.Gondo; 
