...
htot = length(c_L)*length(bal_mas)*length(flap)*length(flap)*length(flap)*length(flap);
for c_l=1:length(c_L)
    for m_B=1:length(bal_mas)
        for qwa=1:length(flap) %%% flap delfection
            for qwf3=1:length(flap)
                ...
                % electronics package
                runs.elecs = bal_mas(m_B);   % [kg] electronics mass
...
% %                 % flap deflection, [boom to tip flaps][F1, F2, F3] 
% %                 wing.d_fps = [15, 15, -15];
% %                 wing.d_ail = -15;
% %                 % ballast mass
% %                 runs.elecs = 1.4;   % kg, ballast mass
                ...
                const.CL = c_L(c_l);       % lift coefficient constraint
                const.b = 0;        % beta
                const.r = 0;        % roll rate
                ...
                if runs.fsFlag == 1
                    ...
                    [hinges] = ReadHinge([runs.name, '.hinge'], design); % read in hinge file
                end
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                hingeV = [hinges.ailer, hinges.flap3, hinges.flap2, hinges.flap1, hinges.ELEV];
                Stvalues=[c_L(c_l), rollAngle,runs.elecs,stabDerivs.Alpha,stabDerivs.Clptot,stabDerivs.ailer,stabDerivs.flap3,stabDerivs.flap2,stabDerivs.flap1,stabDerivs.elev, yl ];
                if c_l==1
                    MAT1(h,:)=[Stvalues, hingeV];
                else
                    h1=h1+1;
                    MAT2(h1,:)=[Stvalues, hingeV];
                end
