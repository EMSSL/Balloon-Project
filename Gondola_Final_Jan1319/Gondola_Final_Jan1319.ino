...
class Rudder{
  public:
    uint8_t limitSW
    Rudder(uint8_t,uint8_t,int,int);
    void begin(uint8_t);
    float getAngle();
    void driveRudder(float);
  private:
    int degreesLow;
    int degreesHigh;
    uint8_t pwmPin;
    ...
    uint16_t POT_ZERO;
    Servo rudderServo;
    uint16_t angle_vs_PWM[256];
    uint16_t 256toPWM(uint8_t);
    uint16_t getAngle16();
    uint8_t mapAngle256(float);
    uint8_t mapPot256(uint16_t);
    int map256Pot(uint8_t);
    void driveRudder(uint8_t);
        
}

Rudder::Rudder(uint8_t pwm, uint8_t feedback, int degree_lo, int degree_high)
    {
      pwmPin = pwm;
      feedbackPin = feedback;
      degreesLow = degree_lo * 10;
      degreesHigh = degree_high * 10;

      pinMode(feedbackPin,INPUT);
      pinMode(limitSW,INPUT_PULLUP);
    }

void Rudder::begin(uint8_t degreeStart)
{
  rudderServo.attach(pwmPin);
  pwmMicro = 1500;
 
  while(analogRead(limitSw) > LIMITSW_LO)   // establish the first limit
  {
    rudderServo.write(pwmMicro);
    pwmMicro--;
  }
  POT_NEG = getAngle16;
  while(analogRead(limitSw) > LIMITSW_LO)
  {
    rudderServo.write(pwmMicro);
    pwmMicro++;
  }
  POT_POS = getAngle16;
  POT_ZERO = (POT_POS + POT_NEG)/2;
  ...
  angle_vs_PWM[255] = pwmMicro - 1;
  while(mapPot256(getAngle16()) == 0xFF)
  {
    pwmMicro--;
    rudderServo.write(pwmMicro);
  }
  
  last256; = mapPot256(getAngle16());
  for(int i = 254; i >= 0; i--)
  {
    current256 = mapPot256(getAngle16());
    while(current256 == last256)
    {
      rudderServo.write(pwmMicro);
      count++;
      sum += (uint32_t)getAngle16();
      pwmMicro--;
    }
    angle_vs_PWM[i] = mapPot256((uint16_t)(sum / count));
  }
    
  Rudder::driveRudder(degreesStart);
}
    
float Rudder::getAngle()
  {
    int angleRaw = analogRead(feedbackPin);
    int angleMapped = (int)map(angleRaw, POT_NEG, POT_POS, degreesLow, degreesHigh);
    float final_angle = ((float)angleMapped) / ((float)10.0);
    return final_angle;
  }

uint16_t Rudder::getAngle()
{
  return(analogRead(feedbackPin));
}
    
bool Rudder::driveRudder(float drive_angle)
  {
